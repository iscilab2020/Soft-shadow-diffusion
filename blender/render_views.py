...
# Create Alpha Over node to combine image with black background
alpha_over = nodes.new(type='CompositorNodeAlphaOver')
alpha_over.use_premultiply = True

# Set the background color of Alpha Over node to black
...
# Check each object's material slots
...
for slot in obj.material_slots:
    if slot.material and slot.material.node_tree:
        if 'Principled BSDF' in slot.material.node_tree.nodes:
            ...
            # Handle case where Principled BSDF is not found
            pass
    else:
        # Handle case where there is no material or no node-based material
        pass
...
print()
model_identifier = "-".join(args.obj.split("/")[-4:-2])
folder, file = model_identifier.split("-")[0], model_identifier.split("-")[1]
fp = os.path.join(os.path.abspath(args.output_folder), folder)
...
for i in range(0, args.views):
    ...
    render_file_path = fp+"/"+file + '_r_{0:03d}'.format(int(i * stepsize))
    ...
    # print(albedo_file_output.file_slots[0].path)
    

    # id_file_output.file_slots[0].path = render_file_path + "_id"
    check = os.path.exists(albedo_file_output.file_slots[0].path+"0001.png")
    
    if check:
        cam_empty.rotation_euler[2] += math.radians(stepsize)
        print(not check, albedo_file_output.file_slots[0].path+"0001.png", "Loud")
        continue
    ...
    # time.sleep(0.02)
    # image = Image.open(albedo_file_output.file_slots[0].path+"0001.png").resize((512, 512), Image.LANCZOS)
    # black_background = Image.new('RGBA', image.size, (0, 0, 0, 255))
    # black_background.paste(image, (0, 0), image)
    # black_background.save(albedo_file_output.file_slots[0].path+"0001.png")
...
# image_path = '/mnt/data/03001627-1fccc2ac4bfd3da535836c728d324152_r_120_albedo0001.png'
...
# output_path = '/mnt/data/image_with_black_background.png'

# black_background.save(output_path)


'/media/predstan/DATA/ShapeNetCore.v2/03636649/63538ead3a81058f1c1ef519b9196b63/models/model_normalized.obj'