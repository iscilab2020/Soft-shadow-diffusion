...
class BetaVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = nn.Sequential(*modules)
        self.fc_mu = nn.Conv2d(hidden_dims[-1], latent_dim//3, kernel_size=3, padding=1)  # for plane features resolution 64x64, spatial resolution is 2x2 after the last encoder layer
        self.fc_var = nn.Conv2d(hidden_dims[-1], latent_dim//3, kernel_size=3, padding=1)  if self.kl_std != 'zero_mean' else nn.Identity()


        # Build Decoder
        modules = []

        self.decoder_input = nn.Conv2d(latent_dim//3, hidden_dims[-1], kernel_size=3, padding=1) 
...
class AE(BetaVAE):
    ...
    def __init__(self, in_channels=3, latent_dim=256, hidden_dims=[256, 256] ) -> None:
